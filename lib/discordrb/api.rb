...
module Discordrb::API
  ...
  def user_agent
    ...
    "#{required} faraday/#{Faraday::VERSION} #{RUBY_ENGINE}/#{RUBY_VERSION}p#{RUBY_PATCHLEVEL} discordrb/#{Discordrb::VERSION} #{@bot_name}".strip
  end
  ...
  def request(key, major_parameter, type, *attributes)
    ...
    begin
      ...
      unless mutex.locked?
        ...
        Discordrb::LOGGER.ratelimit("Locking RL mutex (key: #{key}) for #{"%.3f" % wait_seconds} seconds due to Discord rate limiting")
        trace("429 #{key.join(" ")}")
  ...
  def handle_preemptive_rl(headers, mutex, key)
    ...
    Discordrb::LOGGER.ratelimit("Locking RL mutex (key: #{key}) for #{"%.3f" % delta} seconds pre-emptively")
