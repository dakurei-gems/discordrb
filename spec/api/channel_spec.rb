    ...
    it "sends requests" do
      expect(Discordrb::API).to receive(:request)
        .with(
          ...
          channel_id,
          :get,
          "#{Discordrb::API.api_base}/channels/#{channel_id}/messages/#{message_id}/reactions/test",
    ...
    it "percent-encodes emoji" do
      expect(Discordrb::API).to receive(:request)
        .with(
          ...
          channel_id,
          :get,
          "#{Discordrb::API.api_base}/channels/#{channel_id}/messages/#{message_id}/reactions/%F0%9F%91%8D",
          {limit: 27, before: before_id, after: after_id},
          any_args
        )
